...
def load_df(h5_file):
   ...
   df = df.query("y > -1 & mu_dxy < 0.5 & mu_dz < 1 & mu_sip < 4")
#   df = df.query("y > -1")
...
roc_curves = [
              ("2016", "bdt_score_default", "ID+ISO Muon XGBoost MVA"),
             ]
...
#         plt.savefig(join(plot_dir, "ROC/2016_{0}_{1}.eps".format(location, ptrange)), bbox_inches='tight')
         plt.savefig(join(plot_dir, "ROC/2016_{0}_{1}.png".format(location, ptrange)), bbox_inches='tight')
